...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam){
    size_t dma_half_buffer_max = CAMERA_DMA_BUFFER_SIZE_MAX / 2 / cam->dma_bytes_per_item;
    ...
    cam->dma_buffer_size = dma_buffer_size * cam->dma_bytes_per_item; // 25600
    cam->dma_half_buffer_size = dma_half_buffer * cam->dma_bytes_per_item; // 12800
    cam->dma_node_buffer_size = node_size * cam->dma_bytes_per_item; // 3200
    cam->dma_half_buffer_cnt = cam->dma_buffer_size / cam->dma_half_buffer_size; //2
...
//Define el modo de sampleo
esp_err_t ll_cam_set_sample_mode(cam_obj_t *cam)
//, pixformat_t pix_format, uint32_t xclk_freq_hz, uint16_t sensor_pid)
{
