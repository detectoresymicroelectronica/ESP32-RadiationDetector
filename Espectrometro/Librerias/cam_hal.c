...
#define UMB_DETU 20         //Umbral de deteccion para pixeles de evento unico
#define UMB_DETD 4          //Umbral de deteccion para pixeles vecinos
#define UMB_DESC 6          //Umbral de descarte de pixeles defectuosos

#define LONG_HIST 256       //Longitud del histograma
#define SIZE_H 1600         //Longitud de una fila del sensor de imagen
#define CAPTURAS 20         //Cantidad de capturas iniciales descartadas antes de operar

int A[SIZE_H];              //Vector auxiliar para almacenar una fila
int B[SIZE_H];              //Vector auxiliar para almacenar una fila
int C[SIZE_H];              //Vector auxiliar para almacenar una fila

int col = 0;                //Indice de columnas
int row = 0;                //Indice de filas (solo 3)
int pix = 0;                //Valore de cada pixel
...
int histograma[LONG_HIST];        //Histograma principal
int histograma_copia[LONG_HIST];  //Copia del histograma principal para permitir el envio 
...
void descartar_pixeles(const uint8_t* src, size_t len){
    ...
    for (size_t i = 0; i < end; ++i) {
        ...
        dma_el += 8;
            ...
            if(d1 > UMB_DESC && Pixel.Pos < VECTOR_LENGTH ){
                Pixel.Vect[Pixel.Pos] = pixeles;
                Pixel.Pos++;
            }
            pixeles++;

            if(d2 > UMB_DESC && Pixel.Pos < VECTOR_LENGTH ){
                Pixel.Vect[Pixel.Pos] = pixeles;
                Pixel.Pos++;
            }
            pixeles++;

            if(d3 > UMB_DESC && Pixel.Pos < VECTOR_LENGTH ){
                Pixel.Vect[Pixel.Pos] = pixeles;
                Pixel.Pos++;
            }
            pixeles++;

            if(d4 > UMB_DESC && Pixel.Pos < VECTOR_LENGTH ){
                Pixel.Vect[Pixel.Pos] = pixeles;
                Pixel.Pos++;
            }
            pixeles++;
...
//Determina si un pixel cumple con la condicion de evento unico
//y almacena su valor en el histograma.
//Esta funcion opera con los vectores A, B y C para almacenar
//los valores de cada fila
void SingleEventPixel(){

    A[col] = pix;
    
    if(col > 2 && B[col-1] >= UMB_DETU){
        if(B[col-2] < UMB_DETD && B[col] < UMB_DETD){
            if(A[col-1] < UMB_DETD && C[col-1] < UMB_DETD){
                if(A[col-2] < UMB_DETD && A[col] < UMB_DETD){
                    if(C[col-2] < UMB_DETD && C[col] < UMB_DETD){
                        histograma[B[col-1]]++;}}}}
